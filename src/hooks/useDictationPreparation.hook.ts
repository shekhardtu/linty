...
export function useDictationPreparation() {
  const { settingsLoaded, loadedModelFilename, transcriptionLanguage, reformatEnabled, dictionaryEnabled } = useAppStore();
  const hasWords = useAppStore((s) => s.dictionaryEntries.some((entry) => entry.enabled));
  useEffect(() => {
    if (!settingsLoaded || !modelSupportsLanguage(loadedModelFilename, transcriptionLanguage)) return;
    const prepare = () => {
  ...
  }, [settingsLoaded, loadedModelFilename, transcriptionLanguage, reformatEnabled, dictionaryEnabled, hasWords]);
}

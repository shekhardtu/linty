...
import { initialVersion, nextVersion } from '../scripts/prepare-release.mjs';
...
test('initial release selection cannot downgrade or reuse existing version history', () => {
  assert.equal(initialVersion('0.0.1', []), '0.0.1');
  assert.throws(() => initialVersion('0.0.52', []), /source version 0.0.1/);
  for (const tags of [['v0.0.1'], ['v0.0.67'], ['0.0.2']]) {
    assert.throws(() => initialVersion('0.0.1', tags), /no existing version tags/);
  }
});

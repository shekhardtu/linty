...
function preview({ reduced = false } = {}) {
  ...
  const names = ['history', 'overview', 'dictation'];
...
test('rotates every ten seconds in view and wraps without duplicate active screens', () => {
  ...
  p.visible(true);
  for (const name of ['dictation', 'history', 'overview', 'dictation']) {
    p.tick(9999);
...
test('rapid screen clicks keep the final choice and the matching caption', () => {
  ...
  p.finish();
  p.finish();
  assert.equal(p.selected, 'dictation');
  assert.equal(p.ids['screen-caption'].textContent, 'dictation caption');
  assert.equal(p.ids['product-status'].textContent, 'dictation');
  assert.equal(p.slides.filter(slide => !slide.hidden).length, 1);
  assert.equal(p.timers.size, 1);
});

...
export const macTargets = ['aarch64-apple-darwin', 'x86_64-apple-darwin'];
...
export function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    else if (arg === '--initial-release') options.initialRelease = true;
  ...
  if (options.initialRelease && seen.has('--bump')) throw new Error('--initial-release cannot be combined with --bump.');
  return options;
}
...
export function updaterManifest({ version, notes, signature, archive, previous, releaseType = 'optional', now = new Date() }) {
  ...
  return {
    ...
    platforms: Object.fromEntries(['darwin-aarch64', 'darwin-x86_64'].map(platform => [platform, {
...
export function readPreviousManifest({ gh, buildDir, initialRelease = false }) {
  if (initialRelease) {
    // An API failure must stop publication, not be treated as an empty history.
    const releases = JSON.parse(gh(['api', `repos/${repository}/releases?per_page=1`]));
    if (releases.length) throw new Error('Initial publication requires an empty GitHub release history.');
    return {};
  }
  ...
  return JSON.parse(readFileSync(path.join(previousDir, 'latest.json'), 'utf8'));
}

export function publishBuiltRelease({ git, gh, sourceSha, builtSha, version, buildDir, initialRelease = false, requireLocalPublisher, upload }) {
  requireLocalPublisher();
  git(['fetch', 'origin', 'main', '--tags']);
  assertMainMatches(git, sourceSha);
  const tags = git(['tag', '--list']).trim().split('\n');
  if (initialRelease) {
    initialVersion(version, tags);
    if (builtSha !== sourceSha || git(['rev-list', '--count', sourceSha]).trim() !== '1') {
      throw new Error('The initial release must tag the single initial commit unchanged.');
    }
  } else if (git(['rev-parse', `${builtSha}^`]).trim() !== sourceSha) {
    throw new Error('The built version commit has a different source.');
  }
...
async function main(options) {
  if (options.help) {
    console.log('Usage: yarn release:local [--check | --build-only | --publish] [--release-type required|optional] [--bump patch|minor|major | --initial-release]\n' +
  ...
  for (const key of [...signingNames, 'GH_TOKEN', 'GITHUB_TOKEN', 'LINTY_PARAKEET_LIB_DIR', 'LINTY_BUNDLE_DIR', 'CARGO_BUILD_TARGET']) delete baseEnv[key];
  ...
  const installedTargets = run('rustup', ['target', 'list', '--installed'], { capture: true }).trim().split(/\s+/);
  if (macTargets.some(item => !installedTargets.includes(item))) {
    throw new Error(`Install both Mac targets first: rustup target add ${macTargets.join(' ')}`);
  }
  // Execute the Intel tests locally before signing the universal app.
  ...
  try {
    ...
    const version = run(process.execPath, ['scripts/prepare-release.mjs',
      ...(options.initialRelease ? ['--initial-release'] : ['--bump', options.bump]),
    ...
    if (options.initialRelease) {
      if (git(['rev-list', '--count', sourceSha]).trim() !== '1') throw new Error('Initial publication requires a single initial commit.');
    } else {
    ...
    for (const nativeTarget of macTargets) {
      build('cargo', ['test', '--locked', '--manifest-path', 'src-tauri/Cargo.toml', '--target', nativeTarget, '--features', 'local-stt,parakeet']);
    }
    ...
    const files = [
      [`src-tauri/target/release/bundle/dmg/linty-${version}.dmg`, dmg],
      ['src-tauri/target/release/bundle/dmg/linty.dmg', dmg],
    ...
    const previous = readPreviousManifest({ gh, buildDir, initialRelease: options.initialRelease });
